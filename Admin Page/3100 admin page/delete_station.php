...
<div id="wrapper">
    ...
    <!-- Content Wrapper -->
    <div>
        <h2>delete station</h2><br><br>
        <form method="post" action="delete_station.php">
        ...
        if(empty($route_id)){
            ...
            if(mysqli_num_rows($query_run)>0){
                ...
                if(empty($route_order)){
                    ...
                    $query_run=mysqli_query($con,$query);
                    if(mysqli_num_rows($query_run)>0){
                        $query = "DELETE FROM stations_routes WHERE route_id='$route_id' and route_order='$route_order'";
                        $query_run = mysqli_query($con,$query);
                        if($query_run){
                            echo '<script type="text/javascript"> alert("station deleted!")</script> ';

                        }else{
                            echo '<script type="text/javascript"> alert("Error!")</script> ';
                        }
                    }else{
                        echo '<script type="text/javascript"> alert("the order of this route no. does not exist,please got to update station page ")</script> ';
                    }
                }
            }else{
