...
<div id="wrapper">
    ...
    <div>
        ...
        <form method="post" action="add_route.php">
            ...
            <input type="text" name="route_no" placeholder="Type the route no."><br><br>
            <label for="fname" class="label-align">bus no :</label>
            <input type="text" name="bus_no" placeholder="Type the bus no."><br><br>
    ...
    if(isset($_POST['submit'])){
        ...
        if(mysqli_num_rows($query_run)>0){
            echo '<script type="text/javascript"> alert("the bus no. has been already exist ,bus no. should be unique ,please go to edit page ")</script> ';
        }else{
            $route_no=$_POST['route_no'];
            $query="select * from bus WHERE route_no='$route_no'";
            $query_run=mysqli_query($con,$query);
            if(mysqli_num_rows($query_run)>0){
                echo '<script type="text/javascript"> alert("the rout no. has been already exist ,route no. should be unique ,please go to edit page ")</script> ';
            }else{
                $query = "insert into bus values('$bus_no','$route_no')";
                $query_run = mysqli_query($con,$query);

                if($query_run){
                    echo '<script type="text/javascript"> alert("route add go to add station page")</script> ';
                }else{
